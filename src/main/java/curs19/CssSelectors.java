...
public class CssSelectors extends BaseTest {
	...
	public void cssSelectorsExample() {
		 ...
		 * 
		 * 
		 * . --> in css "punct" este o referinta catre atributul class
		 * 
	...
	public void cssSelectorExample2() {
		...
		jse.executeScript("arguments[0].setAttribute('style', 'border: 4px solid red')", bookTitle);
		...
		jse.executeScript("arguments[0].setAttribute('style', 'background: orange; border: 4px solid red')", bookTitle3);
		
		// $ --> ends-with
		WebElement bookTitle4 = driver.findElement(By.cssSelector("a[href$='silence']"));
		jse.executeScript("arguments[0].setAttribute('style', 'background: orange; border: 4px solid red')", bookTitle4);
